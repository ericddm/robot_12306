...
实现：通过splinter.browser模拟浏览器登陆和操作（抢票过程中，浏览器不可关闭）
抢票成功，会进行邮件的通知
...
class RobotTicket(object):
    """买票类及实现方法"""

    def __init__(self, passengers_num):
        """定义实例属性，初始化"""
        cp= ConfigParser()
        cp.read("resources/conf/city.conf", encoding='UTF-8')
        ...
        cp= ConfigParser()
        cp.read("resources/conf/passenger.conf", encoding='UTF-8')
        sections = cp.sections()
        pessenger = sections[passengers_num]
...
if __name__ == '__main__':
    ...
    pessenger_num = int(input('请输入乘客配置参数编号：'))

    # 开始抢票
    ticket = RobotTicket(pessenger_num)
    ticket.start_brush()
